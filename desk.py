...
class Desk:
	...
	def put_card(self, index):
		if not self.cards_on_table or self.check_cards(self.get_current_player().get_cards()[index]):
			...
			self.send_msg_to_all("You cannot podkinut etu kartu")
			self.attack()
	
	def defend_card(self, index, card):
		if self.check_defend_cards(card):
			self.cards_on_table.append(self.players[self.current_player].get_card(index))
			self.send_msg_to_all("Player {} otbilsy kartoy {}".format(self.get_current_player().get_user_name(), self.cards_on_table[-1]))
			#print("error 6")
			#self.send_msg_to_all(self.cards_on_table[-1])
			#print("error 7")
			self.change_player()
		else:
			self.send_msg_to_all("you cannot bittn this card")
			self.deffend()
	...
	def podbros(self):
		...
		while True:
			try:
				self.players.send_msg("Exit - to finish")
	...
	def flip_card(self):
		...
		if a == 0:
			...
			try:
				...
				self.send_msg_to_all("Player {} najal ne tu knopku".format(self.get_current_player().get_user_name()))	
	...
	def attack(self):
		...
		if "put" == put:
			...
			self.send_msg_to_all("Player {} pohodil kartoy {}".format(self.get_current_player().get_user_name(), self.players[self.current_player].get_cards()[index]))
			#print("error 2")
			self.put_card(index)
			self.change_player()
	...
	def deffend(self):
		...
		if "put" == put:
			...
			self.send_msg_to_all("Player {} cannot bit this cards.".format(self.get_current_player().get_user_name()))
			#print("error 1")
			self.flip_card()
			self.change_player()
	...
	def podbros(self):
		while True:
			...
			self.send_msg_to_all("Player {} doljen podbrosit kartu ili vvesti 'end' dla okonchania podbrose".format(self.get_current_player().get_user_name()))
			#print("error 5")
			...
			if "put" == put:
				...
				self.send_msg_to_all("Player {} zakonchil podbros".format(self.get_current_player().get_user_name()))
				#print("error 3")
				self.counter=1
				return
